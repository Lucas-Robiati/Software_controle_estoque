...
class Application(Validate):
    ...
    def layout_produtos(self):
        ...
        # Frame dos botões
        self.frame_botoes = Frame(self.grid_produto, bg=Color.white.value)
        self.frame_botoes.grid(row=7, column=0, columnspan=2, pady=(0, 10), sticky="ew")
        self.frame_botoes.grid_columnconfigure((0, 1, 2), weight=1)

        self.btn_add_produto = self.create_button(self.frame_botoes, "Adicionar", command_func=self.add_produto)
        self.btn_add_produto.grid(row=0, column=0, padx=5)

        self.btn_editar_produto = self.create_button(self.frame_botoes, "Atualizar", command_func=self.editar_produto)
        self.btn_editar_produto.grid(row=0, column=1, padx=5)

        self.btn_remover_produto = self.create_button(self.frame_botoes, "Deletar", command_func=self.deletar_produto)
        self.btn_remover_produto.grid(row=0, column=2, padx=5)
    ...
    def editar_produto(self):
        ...
        try:
            ...
            print(f"[DEBUG] update_produto resultado: {resultado}")
